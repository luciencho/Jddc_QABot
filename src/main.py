...
from src.prepro import load_vocab, load_data, Batch, tokenizer
from src.model import GraderBase, GraderModel


vocab_path = os.path.join(tmp_dir, 'train_q.vcb')
train_question_path = os.path.join(tmp_dir, 'train_q.txt')
train_answer_path = os.path.join(tmp_dir, 'train_a.txt')
dev_question_path = os.path.join(tmp_dir, 'dev_q.txt')
dev_answer_path = os.path.join(tmp_dir, 'dev_a.txt')
model_path = os.path.join(tmp_dir, 'model.ckpt')
vec_path = os.path.join(tmp_dir, 'result.vec.npz')
ann_path = os.path.join(tmp_dir, 'question_vecs.ann')


def train(hparam):
    word2id, _ = load_vocab(vocab_path)

    train_question = load_data(train_question_path)
    train_answer = load_data(train_answer_path)
    train_batch = Batch(hparam.batch_size, hparam.x_max_len, hparam.y_max_len,
                        train_question, train_answer, word2id)

    dev_question = load_data(dev_question_path)
    dev_answer = load_data(dev_answer_path)
    dev_batch = Batch(hparam.batch_size, hparam.x_max_len, hparam.y_max_len,
                      dev_question, dev_answer, word2id)

    model = GraderModel(hparam)
...
def build_vector(hparam):
    ...
    with tf.Session() as sess:
        ...
        np.savez(vec_path, question_vecs=question_vecs, answer_vecs=answer_vecs)
        annoy_with_npz()
...
class Answer(object):
    def __init__(self, hparam):
        ...
        self.answer = load_data(train_answer_path)
        self.model = GraderModel(hparam)
        self.sess = tf.Session()
        saver = tf.train.Saver()
        saver.restore(self.sess, model_path)
        self.word2id, _ = load_vocab(vocab_path)
...
if __name__ == '__main__':
    # train(GraderBase())
    # build_vector(GraderBase())
    answer = Answer(GraderBase())
